...
use shared::models::{categories::perfume::{PerfumeQuery, PerfumeTile}, wish_list::{WishListRequest, WishListStatus}};

use yew::{function_component, html, platform::spawn_local, use_context, use_effect_with, use_state, Callback, Html, Properties};
...
pub fn catalog_tile(props: &CatalogTileProps) -> Html {
    ...
    let token = use_state(|| LocalStorage::get::<String>("Token"));
    let is_wish_listed = use_state(|| WishListStatus::Unknown);

    use_effect_with(
        (),
        {
            let product_page_src = props.product_page_src.clone();
            let token = token.clone();
            let is_wish_listed = is_wish_listed.clone();

            move |()| {
                let product_page_src = product_page_src.clone();
                let token = token.clone();
                let is_wish_listed = is_wish_listed.clone();
                ...
                spawn_local(async move {
                    match token.deref() {
                        Ok(t) => {
                            let json = WishListRequest { token: t.to_owned(), item_id: product_page_src };
                            let fetched_response = Request::post("https://localhost:5000/wish_list_check")
                            ...
                            match fetched_response {
                                Ok(response) => {
                                    match response.json::<bool>().await {
                                        Ok(b) => {
                                            if b {
                                                is_wish_listed.set(WishListStatus::Wished);
                                                log!("TRUE");
                                            } else {
                                                is_wish_listed.set(WishListStatus::NotWished);
                                                log!("FALSE");
                                            }
                                        },
                                        Err(_) => { }
        ...
        Callback::from(move |_| {
            ...
            spawn_local(async move {
                ...
                match token.deref() {
                    Ok(t) => {
                        let json = WishListRequest { token: t.to_owned(), item_id: product_page_src };
                        let fetched_response = Request::post("https://localhost:5000/wish_list_add")
                        .header("Authorization", &format!("Bearer {}", t))
                        .json(&json)
                        .unwrap()
                        .send()
                ...
                <div class="goods-tile-inner">
                    <div class="goods-tile__content">
                        ...
                        <div class="goods-tile__actions">
                            ...
                            // WishLists are only available for Authed users
                            if { *client_context.is_auth.deref() == IsAuth::Yes } {
                                <div class="wish-wrapper">
                                    <button class="wish-button" aria-label="Add to the wish-list" onclick={wish_list_add}>
                                        <svg width="24" height="24" aria-hidden="true">
                                            <use href="#icon-heart-empty">
                                                <symbol viewBox="0 0 24 24" id="icon-heart-empty">
                                                    <WishListSvg wish_list_status={is_wish_listed.deref().to_owned()} />
                                                </symbol>
                                            </use>
                                        </svg>
                                    </button>
